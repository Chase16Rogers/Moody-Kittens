...
1. Handle Page Refresh
  ...
  - Draw the kittens list
    ...
    let petMoodSet = kittens.affection
    let petMood = Math.random()
    if (petMood > .7) { let petMoodSet = kittens[id].affection++ } else { let petMoodSet = kittens[id].affection-- }
    document.getElementById("catAffectionD").innerHTML = petMoodSet

       if (petIndex == -1) { console.log("Failed to find cat for pets") }
  if (Math.random() > .7) { kittens[petIndex].affection += 1 }
  else {kittens[petIndex].affection -= 1}